...
Script computes MD5 hashes for local files, filtered by `cwd`/`pattern` parameters combination and builds so-called map of hashes.
...
If there are already objects in bucket, script looks for a file with hashes map. It will be used in order to detect the difference between local and S3 states.
If none is found, ETags of S3 objects are used. **Keep an eye on that as AWS may fill ETag with non-MD5 value in certain cases. See [AWS Docs](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTCommonResponseHeaders.html) on that**
