...
[![Node.js min version](https://img.shields.io/node/v/s3-redeploy.svg)](https://www.npmjs.com/package/s3-redeploy)
[![Module version](https://img.shields.io/npm/v/s3-redeploy.svg)](https://www.npmjs.com/package/s3-redeploy)
[![Downloads per week](https://img.shields.io/npm/dw/s3-redeploy.svg)](https://www.npmjs.com/package/s3-redeploy)
...
| --prefix       	| no        	| Value that will be prepended (with a '/') with every file name found on cwd. This allows to deploy multiple projects/versions on the same bucket | -          	| --prefix current<br> --prefix v0.30.0<br> --prefix testing |
| --pattern      	| no        	| Glob pattern, applied within the `cwd` directory. The [glob](https://www.npmjs.com/package/glob) module is used to perform this operation. **If no files match the pattern, the script will exit and leave bucket as is. This is done in order to prevent occasional bucket clearance due to wrong `pattern`/`cwd` combination**.                                                                                                        	| './**'                 	| --pattern './\*\*.{js,html}'<br> `**` works as a `globstar`, see [docs](https://www.gnu.org/software/bash/manual/html_node/The-Shopt-Builtin.html) 	|
...
It also uses no transpilers, etc., which means package contains no garbage dependencies and the size is rather small.
...
If no map is found or no dictionary is intentionally used, ETags of S3 objects will be checked to determine possible changes. **Keep in mind that AWS may fill ETag with non-MD5 value in certain cases. See [AWS Docs](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTCommonResponseHeaders.html) on that.**
So, same files may be treated as different when relying on S3 ETags. Anyway, if ETags do not match, a local version of file will be re-uploaded. This way or another, bucket will get the updated file eventually.
...
It drastically increases processing speed and provides an ability to decrease number of S3 requests. It also allows to abstract from AWS' mechanism of ETags computation.
